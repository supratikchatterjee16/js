...
console.log("Hi from your UI library.\nReminder : call prepareUi() for some minor environment adjustments post load.");function prepareUi(){document.body.style.margin="0pt";}
...
connectedCallback(){let position='relative';try{position=this.getAttribute('position');this.style.position=position;}catch(e){console.error('FillComponent error : ',e);}
if(position!='absolute'||position!='fixed'){this.parentNode.style.width="100%";this.parentNode.style.height="100%";this.style.width=this.parentNode.style.width;this.style.height=this.parentNode.style.height;}
else{this.style.width="100%";this.style.height="100%";}
if(this.hasAttribute('center')){this.style.display='flex';this.style.alignItems='center';this.style.justifyContent='center';}
if(this.hasAttribute('force-down')){this.style.height=window.innerHeight-(this.getBoundingClientRect().top+window.scrollY);}}
